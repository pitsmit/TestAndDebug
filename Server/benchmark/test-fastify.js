...
async function runFastifyTest(runNumber, duration = 30, connections = 10) {
    const url = 'http://framework-fastify:3000/api/health';
    console.log(`🧪 Testing FASTIFY (run ${runNumber})...`);

    const result = await autocannon({
        url: url,
        connections: connections,
        duration: duration,
        title: `fastify-health-test-run-${runNumber}`
    });
    ...
    console.log(`✅ FASTIFY run ${runNumber}: ${result.requests.average} req/sec`);
    return testResult;
}
...
async function main() {
    ...
    console.log(`🚀 Starting FASTIFY-only benchmark with ${runs} runs`);

    for (let i = 1; i <= runs; i++) {
        console.log(`\n📊 Run ${i}/${runs}`);
        const result = await runFastifyTest(i, 10, 5);
        results.push(result);
    ...
    fs.writeFileSync('/app/results/fastify-benchmark.json', JSON.stringify(results, null, 2));

    // Отчет
    generateReport(results, 'FASTIFY');
}
